...
def getRQAStats(R, dmin, vmin, do_norm = False):
    """
    ...
    :param do_norm: Whether to normalize features so they're in
...
def getRQAArr(rqa):
    strs = sorted(rqa.keys())
    res = np.array([rqa[s] for s in strs])
    return (res, strs)
